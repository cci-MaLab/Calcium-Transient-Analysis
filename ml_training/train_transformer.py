from ml_training.dataset import LocalTransformerDataset, train_val_test_split
from ml_training.model import LocalTransformer
...
def train(): 
	...
	trainDS = LocalTransformerDataset(train_unit_ids, section_len=config.SECTION_LEN, rolling=config.ROLLING, slack=config.SLACK)
	valDS = LocalTransformerDataset(val_unit_ids)
	testDS = LocalTransformerDataset(test_unit_ids)
	...
	local_transformer = LocalTransformer(dim=3, local_attn_window_size=config.SLACK, 
									  	max_seq_len=2*config.SLACK+config.SECTION_LEN, depth=1, 
										causal=False, look_forward=1, look_backward=1, 
										exact_windowsize=True).to(config.DEVICE)
	# initialize loss function and optimizer

	lossFunc = BCEWithLogitsLoss(pos_weight=trainDS.weight.to(config.DEVICE))
	opt = Adam(local_transformer.parameters(), lr=config.INIT_LR)
	...
	for e in tqdm(range(config.NUM_EPOCHS), leave=False):
		...
		if avgValLoss < lowest_loss:
			...
			name ="local_transformer_model_val_" + current_time + ".pth"
			model_val_path = os.path.sep.join([output_path, name])
			torch.save(local_transformer, model_val_path)
	...
	# Load the best model
	local_transformer = torch.load(model_val_path)
