...
def train(): 
	...
	if config.MODEL_TYPE == "LocalTransformer":
		model = LocalTransformer(dim=3, local_attn_window_size=config.HIDDEN_SIZE, 
		...
		model = BasicTransformer(sequence_len=config.SECTION_LEN, slack=config.SLACK, input_size=3, 
